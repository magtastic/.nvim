...
function Region:add_line_below_region(text)
  local changes = {
    range = self:next_line_lsp_range(),
    newText = text
  }
  vim.lsp.util.apply_text_edits({changes}, self.bufnr)
  local updated_node = self:update_range()
...
function Region:move_node_to_current_position(prefix, postfix)
  ...
  if prefix then
    content[1] = string.format("%s %s", prefix, content[1])
  end

  if postfix then
    table.insert(content, postfix)
  end
...
function Region:get_text()
  local text = vim.api.nvim_buf_get_lines(self.bufnr, self.start_row, self.end_row + 2, false)
