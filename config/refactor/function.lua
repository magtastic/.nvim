...
function Function:move_to_current_position(prefix, postfix)
  local new_region = self.region:move_node_to_current_position(prefix, postfix)
  return setmetatable(
    {
      bufnr = vim.fn.bufnr(),
      region = new_region,
...
function Function:move_function_to_file(last_import_node)
  ...
  if last_import_node then
    ...
    local new_region =
      import_node_region:add_line_below_region(
      string.format("import %s from './%s'", function_name, function_file_name)
    )
    self.update_region(new_region)
  end

  utils.create_file(function_file_name)
  local new_region = self:move_to_current_position(nil, string.format("export default %s", function_name))
