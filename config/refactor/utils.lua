...
M.get_next_arrow_function_node = function()
    ...
    if not node then error("No Treesitter parser found") end

    while node do
        if node:type() == "arrow_function" then break end
        node = node:parent()
    end

    if node:parent():type() == "variable_declarator" and
        node:parent():parent():type() == "lexical_declaration" then
...
M.get_last_import_node = function()
    ...
    for node in root:iter_children() do
        if node:type() == "import_statement" then last_import_node = node end
    end
...
local function find_child_node(current_node, compare_function)
    ...
    if compare_function(current_node) then return current_node end
    local found_node = nil
    for node in current_node.iter_children() do
        found_node = find_child_node(node, compare_function)
        if found_node then return found_node end
    end
end
...
M.get_node_by_id = function(node_id)
    local current_node = ts_utils.get_node_at_cursor()
    local root = ts_utils.get_root_for_node(current_node)
    local function compare(node) return node:id() == node_id end
